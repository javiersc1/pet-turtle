...
class MyFER2013(dsets.FER2013):
    _RESOURCES = {
        "train": ("train.csv", "eda08be5ecaabf18d99a9be94cf2518b"), 
        "test": ("test.csv", "5fe007e06b7caf6a4bc5c4672a0ee237"), 
...
def get_datasets(dataset, transform, root_dir='./data'):
    ...
    if dataset == "food101":
        ...
        train_dataset = DermaMNIST(split="train",transform=transform, download=True, size=224)
        val_dataset = DermaMNIST(split="test",transform=transform, download=True, size=224)

    elif dataset == "OCTMNIST":
        from medmnist import OCTMNIST
        train_dataset = OCTMNIST(split="train",transform=transform, download=True, size=224)
        val_dataset = OCTMNIST(split="val",transform=transform, download=True, size=224)

    elif dataset == "BloodMNIST":
        from medmnist import BloodMNIST
        train_dataset = BloodMNIST(split="train",transform=transform, download=True, size=224)
        val_dataset = BloodMNIST(split="test",transform=transform, download=True, size=224)

    elif dataset == "OrganAMNIST":
        from medmnist import OrganAMNIST
        train_dataset = OrganAMNIST(split="train",transform=transform, download=True, size=224)
        val_dataset = OrganAMNIST(split="test",transform=transform, download=True, size=224)
        ...
        """
            Javier: HuggingFaceM4/Birdsnap dataset on HF, download this instead
